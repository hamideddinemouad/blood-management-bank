...
export const getHospitalRequests = async (req, res) => {
  try {
    ...
    const requests = await BloodRequest.find({
      hospitalId
    }).populate("labId", "name email phone address").sort({
      createdAt: -1
    });
    res.json({
      success: true,
...
export const getHospitalDashboard = async (req, res) => {
  try {
    const hospitalId = req.user._id;
    const [inventory, requests, hospital] = await Promise.all([Blood.find({
      hospital: hospitalId
    }), BloodRequest.find({
      hospitalId
    }).populate("labId", "name").sort({
      createdAt: -1
...
export const getAllDonors = async (req, res) => {
  try {
    ...
    const [donors, total] = await Promise.all([Donor.find(filter).select('fullName email phone bloodGroup lastDonationDate donationHistory address').sort(sortOptions).skip(skip).limit(parseInt(limit)), Donor.countDocuments(filter)]);
    const threeMonthsAgo = new Date();
    threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);
    const [availableDonors, rareBloodDonors] = await Promise.all([Donor.countDocuments({
      $or: [{
        lastDonationDate: {
          $lt: threeMonthsAgo
        }
