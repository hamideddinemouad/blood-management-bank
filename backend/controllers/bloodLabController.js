...
export const getBloodLabDashboard = async (req, res) => {
  try {
    ...
    const [camps, stock, facility] = await Promise.all([BloodCamp.find({
      hospital: labId
    }).sort({
      createdAt: -1
    }), Blood.find({
...
export const getBloodLabHistory = async (req, res) => {
  try {
    ...
    const activity = lab.history.filter(i => ["Blood Camp", "Verification", "Login", "Stock Update"].includes(i.eventType)).sort((a, b) => new Date(b.date) - new Date(a.date));
    const logins = lab.history.filter(i => i.eventType === "Login").map(login => ({
      date: login.date,
...
export const createBloodCamp = async (req, res) => {
  try {
    ...
    if (campDate < today) return res.status(400).json({
      success: false,
      message: "Camp date cannot be in the past"
    });
    if (time.start >= time.end) return res.status(400).json({
      success: false,
...
export const getBloodLabCamps = async (req, res) => {
  try {
    ...
    const [camps, total] = await Promise.all([BloodCamp.find(filter).sort({
      date: -1
    }).skip(skip).limit(parseInt(limit)), BloodCamp.countDocuments(filter)]);
    res.json({
      success: true,
      data: {
        camps,
...
export const getLabBloodRequests = async (req, res) => {
  try {
    ...
    const requests = await BloodRequest.find({
      labId
    }).populate("hospitalId", "name email phone address").sort({
      createdAt: -1
    });
    res.status(200).json({
      success: true,
