...
const DonorDashboard = () => {
  ...
  const handleDownloadCertificate = () => {
    if (!donor) {
      toast.error("Donor profile is still loading");
      return;
    }
...
This certifies that ${donor.fullName} is a registered blood donor.

Blood Group: ${donor.bloodGroup || "N/A"}
Total Donations: ${totalDonations}
Lives Impacted: ${totalDonations * 3}
Donor Since: ${donor.createdAt ? new Date(donor.createdAt).getFullYear() : new Date().getFullYear()}
Issued On: ${certificateDate}

Thank you for supporting the donor community and helping save lives.
    `.trim();

    const blob = new Blob([certificateContent], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    const safeName = (donor.fullName || "donor")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "");

    link.href = url;
    link.download = `${safeName}-donation-certificate.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);

    toast.success("Donation certificate downloaded");
  };
  ...
  const handleShareAchievement = async () => {
    ...
    const totalDonations = history.length;
    const shareMessage = `${donor.fullName} has completed ${totalDonations} blood donation${totalDonations !== 1 ? "s" : ""} and helped impact up to ${totalDonations * 3} lives through BBMS.`;

    try {
      if (navigator.share) {
        await navigator.share({
          title: "Blood donor achievement",
          text: shareMessage,
        });
  ...
  const handleScheduleDonation = () => {
    navigate("/donor/camps");
    toast.success("Browse available donation camps");
  };

  const handleInviteFriends = async () => {
    const inviteMessage =
      "Join me on Blood Bank Management System and help save lives by becoming a blood donor.";
    ...
    try {
      ...
      console.error("Invite clipboard copy failed:", error);
    }

    window.location.href = `mailto:?subject=${encodeURIComponent("Join me as a blood donor")}&body=${encodeURIComponent(inviteMessage)}`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-white p-4 sm:p-6">
      ...
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <button
          ...
          disabled={refreshing}
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-red-600 transition-colors hover:bg-red-50 disabled:opacity-50 sm:w-auto"
        >
          <RefreshCw
