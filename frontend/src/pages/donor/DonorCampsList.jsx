...
const CampCard = ({ camp, onToggleRegistration, actionLoading }) => {
  const isCompleted = camp.status === 'Completed';
  const isCancelled = camp.status === 'Cancelled';
  const isUpcoming = camp.status === 'Upcoming';
  const isRegistered = Boolean(camp.isRegistered);
  const isOpenForRegistration = isUpcoming || camp.status === "Ongoing";
  ...
  const registeredCount = camp.registeredCount || 0;
  const slotsAvailable =
    typeof camp.remainingSlots === "number"
      ? camp.remainingSlots
      : expectedDonors > 0
        ? expectedDonors - registeredCount
        : 0;
    ...
    if (isUpcoming) {
      return (
        <span className="font-medium text-gray-600">
          {registeredCount} registered / {expectedDonors} capacity
        </span>
      );
    } 
    ...
    <div className={`bg-white rounded-2xl shadow-lg transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl p-6 border-2 overflow-hidden ${
      ...
      <div className="pt-4 border-t border-gray-100 flex flex-col justify-between items-start gap-3">
        ...
        {isOpenForRegistration && (
          <div className="w-full rounded-2xl border border-red-100 bg-red-50/60 p-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-sm">
                <p className="font-semibold text-gray-800">
                  {isRegistered ? "You already applied to this camp" : "Ready to join this camp?"}
                </p>
                <p className="text-gray-600">
                  {isRegistered
                    ? "You can cancel if your plans change."
                    : isFull
                      ? "Capacity has been reached for now."
                      : "Reserve your spot as a donor in one click."}
                </p>
              </div>
              <button
                onClick={() => onToggleRegistration(camp)}
                disabled={actionLoading || (!isRegistered && isFull)}
                className={`inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                  isRegistered
              ...
              >
                ...
                {isRegistered ? "Cancel Application" : "Apply To Donate"}
              </button>
            </div>
          </div>
        )}
...
export const DonorCampsList = () => {
  ...
  const handleToggleRegistration = async (camp) => {
    const token = localStorage.getItem("token");

    setActionLoadingId(camp._id);
    ...
    try {
      ...
      setCamps((prev) =>
        prev.map((item) =>
          item._id === camp._id
            ? {
                ...item,
                isRegistered: result?.isRegistered ?? !camp.isRegistered,
                registeredCount:
                  result?.registeredCount ??
                  (camp.isRegistered
                    ? Math.max((item.registeredCount || 1) - 1, 0)
                    : (item.registeredCount || 0) + 1),
                remainingSlots:
                  result?.remainingSlots ??
                  (camp.expectedDonors > 0
                    ? Math.max(
                        camp.expectedDonors -
                          (result?.registeredCount ??
                            (camp.isRegistered
      ...
      toast.success(
        response.data?.message ||
          (camp.isRegistered
            ? "Camp application cancelled"
            : "Applied to donation camp"),
      );
    } catch (err) {
      const message =
        err.response?.data?.message ||
        "Unable to update camp application right now";
      toast.error(message);
    } finally {
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">
                Browse Available Donation Camps
              </h1>
              <p className="text-gray-600 mt-1 text-sm sm:text-base">
                Explore nearby opportunities to donate blood and reserve your spot quickly.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-md border border-red-100 p-4 sm:p-6 mb-6">
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-stretch lg:items-center">
            ...
            <button
              onClick={() => fetchCamps()}
              disabled={loading}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 font-medium text-red-700 transition-all duration-200 hover:bg-red-100 sm:w-auto sm:min-w-[120px]"
            >
              {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
              {displayedCamps.map((camp) => (
                <CampCard
                  ...
                  actionLoading={actionLoadingId === camp._id}
                />
              ))}
            </div>
