...
export const getApiBaseUrl = () => {
  ...
  try {
    ...
    // Prefer same-origin API routing when the app is opened from a LAN/deployed host
    // but the environment still points at a localhost-style backend.
    if (!isLocalHostname(currentOrigin.hostname) && isLocalHostname(configuredOrigin.hostname)) {
      return "";
    }
  } catch {
    return configuredBaseUrl;
